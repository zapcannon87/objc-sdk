...
NS_ASSUME_NONNULL_BEGIN
    ...
    #define LCDBAutorelease(__v) ([__v autorelease]);
    #define LCDBReturnAutoreleased LCDBAutorelease

    #define LCDBRetain(__v) ([__v retain]);
    #define LCDBReturnRetained LCDBRetain

    #define LCDBRelease(__v) ([__v release]);

    #define LCDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define LCDBAutorelease(__v)
    #define LCDBReturnAutoreleased(__v) (__v)

    #define LCDBRetain(__v)
    #define LCDBReturnRetained(__v) (__v)

    #define LCDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define LCDBDispatchQueueRelease(__v)
    #else
        #define LCDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^LCDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);
...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `LCDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<LCResultSet>` - Represents the results of executing a query on an `LCDatabase`.
 - `<LCDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<LCDatabasePool>` - A pool of `LCDatabase` objects.
 - `<LCStatement>` - A wrapper for `sqlite_stmt`.
 ...
 @warning Do not instantiate a single `LCDatabase` object and use it across multiple threads. Instead, use `<LCDatabaseQueue>`.
...
/** Create a `LCDatabase` object.
 
 An `LCDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `LCDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `LCDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    LCDatabase *db = [LCDatabase databaseWithPath:@"/tmp/tmp.db"];
 ...
 @return `LCDatabase` object if successful; `nil` if failure.
...
/** Create a `LCDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `LCDatabase` connection is closed.
...
/** Initialize a `LCDatabase` object.
...
/** Initialize a `LCDatabase` object.
 ...
 An `LCDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(NS_NOESCAPE LCDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<LCResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[LCResultSet next]>`) from one record to the other.
 ...
 @return A `<LCResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see LCResultSet
 @see [`LCResultSet next`](<[LCResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (LCResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (LCResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (LCResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> LCResultSet!`
...
- (LCResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (LCResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (LCResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
- (NSError * _Nullable)inSavePoint:(void (NS_NOESCAPE ^)(BOOL *rollback))block;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    LCResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [LCDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using LCDatabase::storeableDateFormat.
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `LCStatement`, but rather with `<LCDatabase>` and `<LCResultSet>` only.
