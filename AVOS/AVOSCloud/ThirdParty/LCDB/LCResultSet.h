...
/** Represents the results of executing a query on an `<LCDatabase>`.
...
@property (nonatomic, retain, nullable) LCDatabase *parentDB;
...
/** `LCStatement` used by result set. */

@property (atomic, retain, nullable) LCStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<LCStatement>`
 
 @param statement A `<LCStatement>` to be performed
 
 @param aDB A `<LCDatabase>` to be used
 
 @return A `LCResultSet` on success; `nil` on failure
 */
